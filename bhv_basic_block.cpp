...
bool
Bhv_BasicBlock::execute( PlayerAgent * agent )
{
    ...
    std::vector< int > tm_blockers = get_blockers( agent );
    const int self_unum = wm.self().unum();

    if ( tm_blockers.empty()
         || std::find( tm_blockers.begin(),
                       tm_blockers.end(),
                       self_unum ) == tm_blockers.end() )
    {
        last_block_cycle = -1;
        return false;
    }

    std::pair< int, Vector2D > best_blocker_target
        = get_best_blocker( agent, tm_blockers );
...
std::vector< int >
Bhv_BasicBlock::get_blockers( const PlayerAgent * agent )
{
    const WorldModel & wm = agent->world();
    const int opp_min = wm.interceptTable().opponentStep();
    ...
    for ( const PlayerObject * tm : wm.ourPlayers() )
    {
        if ( tm->isGhost() ) continue;
        if ( tm->goalie() ) continue;
        if ( tm->isTackling() ) continue;
        if ( tm->pos().dist( ball_inertia ) > 40.0 ) continue;

        const Vector2D tm_home_pos = Strategy::i().getPosition( tm->unum() );
        if ( tm_home_pos.dist( ball_inertia ) > 40.0 ) continue;
        ...
        if ( tm->unum() <= 5 )
        {
            ...
            for ( int i = 2; i <= 5; ++i )
            {
                const PlayerObject * defensive_tm = wm.ourPlayer( i );
                if ( defensive_tm
                     && defensive_tm->unum() > 0
                     && defensive_tm->pos().x < tm_defense_line_x )
                {
...
Bhv_BasicBlock::get_best_blocker( const PlayerAgent * agent,
                                  std::vector< int > & tm_blockers )
{
    const WorldModel & wm = agent->world();
    const int opp_min = wm.interceptTable().opponentStep();
    ...
    {
        const AngleDeg dribble_dir = dribble_direction_detector( ball_inertia );
        ball_inertia += Vector2D::polar2vector( dribble_speed, dribble_dir );

        for ( const int tm_unum : tm_blockers )
        {
            const PlayerObject * tm = wm.ourPlayer( tm_unum );
            if ( ! tm ) continue;
...
AngleDeg
Bhv_BasicBlock::dribble_direction_detector( Vector2D dribble_pos )
{
    ...
    {
        const Vector2D target = dribble_pos + Vector2D::polar2vector( dist, AngleDeg( dir ) );
        if ( target.absX() > ServerParam::i().pitchHalfLength() ) continue;
        if ( target.absY() > ServerParam::i().pitchHalfWidth() ) continue;

        const double score = -target.x
                             + std::max( 0.0,
                                         40.0 - target.dist( Vector2D( -50.0, 0.0 ) ) );
        if ( score > best_score )
        {
            best_score = score;
